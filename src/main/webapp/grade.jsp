...
<html><c:set var="u_" value="${sessionScope.user}" scope="page"/>
    <head>
        ...
        <script>
            ...
            function modify(obj) {
                ...
                if (new_score === null)
                    return;
                if (new_score.indexOf('.') >= 0||isNaN(parseInt(new_score,10))) { //小数也能parseInt
                ...
                var course=td.prev();
                var sid=course.prev();
                var param="type=2&courseid="+course.attr("value")+"&sid="+sid.attr("value")+"&tid=${u_.id}&score="+new_score;
                $.ajax({
                    url:_root_+"grade", type: "post", data:param, processData: false,  success:function (d) {$("#status").text(d);delayedReload(500);}
                });
            }</c:if>
            function del(obj) {
                ...
                if (confirm("确认删除吗？")) {
                    ...
                    var sid = course.prev();
                    var param="type=1&courseid="+course.attr("value")+"&sid="+sid.attr("value")+"&tid=${u_.id}";
                    $.ajax({
    ...
    <table class="main-table">
        ...
        <tr>
            <% str.setLength(0);
                ...
                str.append(String.format("\t\t<td %s>%d</td>\n",style,s));
            %><%=str.toString()%>
            <td>
                <c:if test="${not u_.admin}">
...
<c:remove var="u" scope="page"/></c:otherwise></c:choose>